...
a different libc.
    ...
    podman build -t hecate-tube -f Containerfile .
...
| `HECATE_NODE_NAME` | `hecate_tube` | Erlang node name. |
| `HECATE_NODE_HOST` | `127.0.0.1` | Erlang node host. |
| `HECATE_COOKIE` | `hecate_tube` | Erlang cookie. |
...
`ghcr.io/hecate-services/hecate-tube:latest` plus the semver tag. Pull `:latest` under
...
Six callbacks in `hecate_tube_service`, all required, all resolved **by name** by
